...
function broadcastToRun(testRunId: string, line: string): void {
  ...
  pipelineEmitters.get(testRunId)?.forEach((emit) => {
    try {
      ...
      // Stream already closed — remove dead emitter to prevent accumulation
      pipelineEmitters.get(testRunId)?.delete(emit);
    }
  });
}
...
async function runPipelineStages(
  ...
  for (const [batchIndex, batch] of chunk(pairs, 50).entries()) {
    ...
    // Count rejected promises (from the real batch execution above) as skipped
...
export async function getTestReportHandler({
  ...
  return buildTestReport(run);
}

// Shared report builder — used by both authenticated and public handlers.
async function buildTestReport(run: NonNullable<Awaited<ReturnType<typeof db.query.test_runs.findFirst<{
  with: { testCases: { with: { results: true } }; bugReports: true; reportExports: true; crawlResult: true };
}>>>>): Promise<object> {
  ...
  const perfRows = await db
    .select()
    .from(performance_metrics)
    .where(eq(performance_metrics.test_run_id, run.id));
...
export async function getPublicReportHandler({
  ...
  // Fetch the run directly — public handler skips auth/ownership checks.
  const run = await db.query.test_runs.findFirst({
    where: eq(test_runs.id, exportRow.test_run_id),
    with: {
      testCases: { with: { results: true } },
      bugReports: true,
      reportExports: true,
